...
describe('makeProcessMessage', () => {
  ...
  it('handles fallback', async () => {
    ...
    await processMessage(ctx);
    expect(bot.telegram.sendMessage).toHaveBeenCalledWith('admin', expect.stringContaining('Msg:'), expect.any(Object));
    expect(bot.telegram.sendMessage).toHaveBeenCalledWith('agent', expect.stringContaining('Msg:'), expect.any(Object));
